          ...
          {isSignedIn ? (
            <>
              {authNavItemsMain.map((item, index) => (
                <Link key={index} href={item.href} className={`h-9 flex items-center gap-2 px-2 sm:px-3 rounded-lg transition-colors ${pathname === item.href ? "text-red-400 bg-red-500/10" : "text-slate-400 hover:text-red-400 hover:bg-red-500/10"}`}>
                  {item.icon}
